	...
	for index, tab in self.tabs do
		...
		if tostring(tab.title):lower() == "skins" then
			...
			loadSleevePreview = function()
				...
				local selected = (team == "CT") and library_flags["skinSleeveCT"] or library_flags["skinSleeveT"]
				local templates = getgenv().CW_SleeveTemplates or env.SleeveTemplates
			...
			applySleeveBtn.MouseButton1Click:Connect(function()
				...
				local applied = false
				pcall(function()
					local applyFunc = getgenv().CW_ApplySleeveChangerToCurrent or env.ApplySleeveChangerToCurrent
					if applyFunc then
						applyFunc()
						applied = true
					end
