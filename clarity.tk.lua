...
local function var_189(var_206, var_9)
	...
	local val_935 = UI_Library:Create("UIListLayout", { Padding = UDim.new(0, 2), Parent = var_206.content }); UI_Library:Create("UIPadding", { PaddingTop = UDim.new(0, 3), PaddingLeft = UDim.new(0, 3), Parent = var_206.content }); local val_936 = 0 
...
function var_206:AddValue(var_243, var_107)
		...
		local val_942 = UI_Library:Create("TextLabel", { ZIndex = 4, Size = UDim2.new(1, 0, 0, 20), BackgroundTransparency = 1, Text = var_243, TextSize = 15, Font = Enum.Font.Code, TextTransparency = self.multiselect and (self.value[var_243] and 1 or 0) or self.value == var_243 and 1 or 0, TextColor3 = Color3.fromRGB(210, 210, 210), TextXAlignment = Enum.TextXAlignment.Left, Parent = var_206.content })
		...
		for var_246, var_141 in val_1013:GetChildren() do
			if library_flags["Weapon Chams"] then
				if var_141:IsA"MeshPart" or var_141.Name == "Part" then
					...
					var_141.Color = library_flags["weaponColor"]; var_141.Transparency = 1 - UI_Library.options["weaponColor"].trans; var_141.Material = library_flags["weaponMaterial"]
			...
			if var_141:IsA"Model" then
				...
				for var_194, var_255 in descendants do
					...
					if library_flags["Arm Chams"] and not isSleeve then
						...
						local val_278 = library_flags["armMaterial"]
						if var_255:IsA"SpecialMesh" then
							if val_278 == "ForceField" then
...
val_792.Name = "bt"
function createPlr(var_35)
	if not var_35.Character then
		return
	end 
	local val_404 = Instance.new("Model", val_792); val_404.Name = var_35.Name 
	if val_814[var_35.Name] then
		val_814[var_35.Name]:Destroy(); val_814[var_35.Name] = nil
	end 
	for var_33, var_258 in var_35.Character:GetChildren() do
		if not var_258:IsA"BasePart" or var_258.Name == "Gun" then
			continue 
		end 
		if var_258.Name ~= "HeadHB" and var_258.Transparency == 1 then
			continue 
		end 
		local val_405 = var_258:Clone(); val_405.Transparency = 0; val_405.Anchored = true; val_405.CanCollide = false; val_405.Color = Color3.new(1, 1, 1); val_405.Size = val_405.Size - Vector3.new(0.035, 0.035, 0.035); val_405.Transparency = 1; val_405.Name = var_258.Name; val_405.Parent = val_404 
		for var_58, var_261 in val_405:GetChildren() do
			if not var_261:IsA"BasePart" and not var_261:IsA"DataModelMesh" then
				var_261:Destroy()
			end
		end
	end 
	if # val_404:GetChildren() > 0 then
		val_814[var_35.Name] = val_404
	end
end 
val_931 = false 
function selfChams()
	if library_flags["Self Chams"] and val_733.Character then
		...
		for _, part in val_733.Character:GetDescendants() do
			...
			if part:IsA("SpecialMesh") then
				local c = library_flags["selfChamsColor"]; part.VertexColor = Vector3.new(c.R, c.G, c.B)
			end
...
function updateSkybox()
	if val_699:FindFirstChild("SunRays") then
	...
	elseif not library_flags["Skybox Changer"] and val_931 then
		if val_699:FindFirstChild("customsky") then
			val_699.customsky:Destroy()
		end 
        ...
        if not checkcaller() and (var_144 == "Text" or var_144 == "text") and (var_13:IsA("TextLabel") or var_13:IsA("TextBox")) then
            if library_flags["spoofName"] then
                ...
                if spoofName and spoofName ~= "" then
                    if type(var_36) == "string" then
                        if var_36:find(val_414.Name) then
                            var_36 = var_36:gsub(val_414.Name, spoofName)
                        end
                        if val_414.DisplayName and var_36:find(val_414.DisplayName) then
                            var_36 = var_36:gsub(val_414.DisplayName, spoofName)
                        end
                    end
                end
            end
...
function val_440:GetTabs()
		...
		if val_438 == "SetPrimaryPartCFrame" or val_438 == "PivotTo" or val_438 == "pivotTo" then
			if val_439 ~= val_414.Name and env._tpActive then
				return oldNamecall(var_270, CFrame.new(0, 0, 0), select(2, ...))
			end
...
masterSec:AddList({
	text = "weapon", values = { "Rifles", "Scout", "AWP", "Heavy Pistols", "Pistols", "SMG", "Other" }, max = 10,
	callback = function(L_1298_arg0)
		rifles2.main.Visible = L_1298_arg0 == "Rifles"; scout2.main.Visible = L_1298_arg0 == "Scout"; awp2.main.Visible = L_1298_arg0 == "AWP"; heavyp2.main.Visible = L_1298_arg0 == "Heavy Pistols"; pistols2.main.Visible = L_1298_arg0 == "Pistols"; smg.main.Visible = L_1298_arg0 == "SMG"; other2.main.Visible = L_1298_arg0 == "Other"
	end
})
...
game:GetService("RunService").RenderStepped:Connect(function()
	...
	for _, plr in val_711:GetPlayers() do
		...
		if char then
			...
			local hrp = char:FindFirstChild("HumanoidRootPart"); local head = char:FindFirstChild("Head"); local params = RaycastParams.new(); params.FilterType = Enum.RaycastFilterType.Exclude
			params.FilterDescendantsInstances = {val_733.Character, char, cam}
			local seen = false
			if hrp then
				local dir = (hrp.Position - camPos); local hit = workspace:Raycast(camPos, dir, params)
				if hit == nil or hit.Instance:IsDescendantOf(char) then seen = true end
			end
			if not seen and head then
				local dir = (head.Position - camPos); local hit = workspace:Raycast(camPos, dir, params)
				if hit == nil or hit.Instance:IsDescendantOf(char) then seen = true end
			end
			for _, part in char:GetDescendants() do
...
UI_Library.options["Flags ESP"]:AddList({
	text = "Flags", flag = "flagsList", multiselect = true, value = { ["Armored"] = true, ["Unarmored"] = true, ["Helmet"] = true, ["Defuser"] = true, ["Bomb"] = true, ["Scoped"] = true, ["Planting"] = true, ["Slowed"] = true }, values = { "Armored", "Unarmored", "Helmet", "Defuser", "Bomb", "Scoped", "Planting", "Slowed" }
})
...
UI_Library.options["Weapon Chams"]:AddList({ flag = "weaponMaterial", values = { "SmoothPlastic", "ForceField", "Neon", "Glass" }, callback = updateViewModelVisuals })
...
UI_Library.options["Arm Chams"]:AddList({ flag = "armMaterial", values = { "SmoothPlastic", "ForceField", "Glass" }, callback = updateViewModelVisuals }); visualsGlow:AddSlider({ text = "Weapon Reflectance", flag = "weaponReflectance", max = 100, callback = updateViewModelVisuals })
...
worldCol1 = visualsGame:AddColumn(); worldCol2 = visualsGame:AddColumn(); lightingSection = worldCol1:AddSection"Lighting"; fogSection = worldCol1:AddSection"Fog"; skySection = worldCol1:AddSection"Sky"; bloomSection = worldCol1:AddSection"Bloom"; atmosphereSection = worldCol2:AddSection"Atmosphere"; terrainSection = worldCol2:AddSection"Terrain"
dofSection = worldCol2:AddSection"Depth of Field"
sunRaysSection = worldCol2:AddSection"Sun Rays"; uiLoaded = false
task.spawn(function()
	task.wait(1.5); uiLoaded = true
end)
...
dofSection:AddSlider({
	text = "Focus Distance", min = 0, max = 1000, float = 1, value = 0,
...
sunRaysSection:AddSlider({
	text = "Intensity", min = 0, max = 1, float = 0.01, value = 0.2,
...
CreateThread(function()
	while true do
		...
		L_ljText.Transparency = trans.lj; L_ljText.Visible = trans.lj > 0; local ebActive = library_flags["showEBInd"] and (val_830 or env.mimicWasFalling or env.helltracingEdgeDetected); local ebTarget = ebActive and 1 or 0; trans.eb = trans.eb + (ebTarget - trans.eb) * lerpSpeed
		if math.abs(trans.eb - ebTarget) < 0.001 then trans.eb = ebTarget end
		L_ebText.Transparency = trans.eb; L_ebText.Visible = trans.eb > 0; local jbActive = library_flags["showJBInd"] and (env.jbBindHeld or (tick() - (env.jbTriggerTime or 0) < 0.4)); local jbTarget = jbActive and 1 or 0; trans.jb = trans.jb + (jbTarget - trans.jb) * lerpSpeed
		if math.abs(trans.jb - jbTarget) < 0.001 then trans.jb = jbTarget end
		L_jbText.Transparency = trans.jb; L_jbText.Visible = trans.jb > 0; L_jbText.Color = env.hasBoosted and Color3.new(0, 1, 0.5) or Color3.new(1, 1, 1); local asAnchored = val_872.alive and val_733.Character and val_733.Character:FindFirstChild("HumanoidRootPart") and val_733.Character.HumanoidRootPart.Anchored; local asActive = library_flags["showASInd"] and asAnchored; local asTarget = asActive and 1 or 0; trans.as = trans.as + (asTarget - trans.as) * lerpSpeed; local tbActive = library_flags["showTBInd"] and env.tbSurfing and library_flags["Texturebug"]; local tbTarget = tbActive and 1 or 0; trans.tb = (trans.tb or 0) + (tbTarget - (trans.tb or 0)) * lerpSpeed
		...
		L_mjText.Transparency = trans.mj; L_mjText.Visible = trans.mj > 0; local fmActiveStatus = library_flags["showFMInd"] and env.fmTriggered; local fmTarget = fmActiveStatus and 1 or 0; trans.fm = trans.fm + (fmTarget - trans.fm) * lerpSpeed
		...
		L_jpText.Transparency = trans.jp; L_jpText.Visible = trans.jp > 0; local wcActive = library_flags["showWCInd"] and (env.wallclimbing or env.wcBindHeld); local wcTarget = wcActive and 1 or 0; trans.wc = (trans.wc or 0) + (wcTarget - (trans.wc or 0)) * lerpSpeed
		if math.abs((trans.wc or 0) - wcTarget) < 0.001 then trans.wc = wcTarget end
		L_wcText.Transparency = trans.wc; L_wcText.Visible = trans.wc > 0; local lbActive = library_flags["showLBInd"] and (env.ladderBugActive or env.lbBindHeld); local lbTarget = lbActive and 1 or 0; trans.lb = (trans.lb or 0) + (lbTarget - (trans.lb or 0)) * lerpSpeed
		...
		if L_fmText.Visible then
			L_fmText.Position = Vector2.new(centerX, baseY + slot * spacing); L_fmText.Color = env.fmTriggered and Color3.new(0, 1, 0) or Color3.new(1, 1, 1)
			if trans.fm > 0.5 then slot = slot + 1 end
		end
		if L_wcText.Visible then
			L_wcText.Position = Vector2.new(centerX, baseY + slot * spacing); L_wcText.Color = (env.wallclimbing or env.wcBindHeld) and Color3.new(0, 1, 0) or Color3.new(1, 1, 1)
			if trans.wc > 0.5 then slot = slot + 1 end
		end
		if L_lbText.Visible then
			L_lbText.Position = Vector2.new(centerX, baseY + slot * spacing); L_lbText.Color = (env.ladderBugActive or env.lbBindHeld) and Color3.new(0, 1, 0) or Color3.new(1, 1, 1)
			if (trans.lb or 0) > 0.5 then slot = slot + 1 end
		end
	end
end)
miscMeme:AddToggle({ text = "Velocity Indicator" }); strafedir = Vector3.new(); cachedMovementIcon = nil
...
nameSpoofSec = generalCol1:AddSection("Name Spoofer"); nameSpoofSec:AddToggle({text = "Enabled", flag = "spoofName"}); nameSpoofSec:AddBox({text = "Spoofed Name", flag = "spoofedNameValue"}); visualsViewmodel = generalCol2:AddSection"Viewmodel"; visualsViewmodel:AddToggle({ text = "Enabled", flag = "viewmodelEnabled" }); visualsViewmodel:AddToggle({ text = "Visualize Silent Angles" })
	...
	env._ebHeartbeat = game:GetService("RunService").Heartbeat:Connect(function()
		...
		if ebWasFalling and ebEdgePos and state == Enum.HumanoidStateType.Landed then
			...
			if landDist < 3.5 and (onEdgeSurface or edgeDelta < 1.1) and vel.Y > -80 then
				...
				local vX, vZ = vel.X, vel.Z; local flatSpeed = Vector3.new(vX, 0, vZ).Magnitude; local boostMult = flatSpeed > 5 and 1.0 or 1.15; local flatDir = Vector3.new(vX, 0, vZ); local moveDir = flatDir.Magnitude > 0.5 and flatDir.Unit or hrp.CFrame.LookVector
				...
				hum:ChangeState(Enum.HumanoidStateType.Freefall); hrp.AssemblyLinearVelocity = Vector3.new(
					vX * boostMult + moveDir.X * 2.5, 0, vZ * boostMult + moveDir.Z * 2.5
				); hrp.CFrame = hrp.CFrame + (moveDir * 0.5); env.hookJP = nil; hum:SetStateEnabled(Enum.HumanoidStateType.Jumping, true); env.lastEdgebugTime = tick()
...
RouteGenerator = {}; RouteGenerator.BEAM_WIDTH = 250; RouteGenerator.MAX_TICKS = 64 * 8
local function RoundVec(v: Vector3, decimals: number)
    local mult = 10 ^ decimals
    return string.format("%.1f_%.1f_%.1f",
        math.floor(v.X * mult + 0.5) / mult, math.floor(v.Y * mult + 0.5) / mult, math.floor(v.Z * mult + 0.5) / mult
    )
end
local function GetActionSpace(pos: Vector3, targetPos: Vector3): table
    local dirToTarget = (targetPos - pos); local flatDir = Vector3.new(dirToTarget.X, 0, dirToTarget.Z)
    if flatDir.Magnitude > 0 then flatDir = flatDir.Unit else flatDir = Vector3.new(1,0,0) end
    local lookCFrame = CFrame.lookAt(Vector3.zero, flatDir); local strafeRight = (lookCFrame * CFrame.Angles(0, math.rad(-70), 0)).LookVector; local strafeLeft = (lookCFrame * CFrame.Angles(0, math.rad(70), 0)).LookVector
    return { {wishdir = flatDir, jump = false, duck = false, name = "Forward"}, {wishdir = strafeRight, jump = false, duck = false, name = "StrafeRight"}, {wishdir = strafeLeft, jump = false, duck = false, name = "StrafeLeft"}, {wishdir = flatDir, jump = true, duck = false, name = "Jump"}, {wishdir = flatDir, jump = true, duck = true, name = "DuckJump"} }
end
function RouteGenerator:GenerateRoute(startPos: Vector3, targetPos: Vector3, targetNormal: Vector3, targetType: string)
    targetNormal = targetNormal or Vector3.new(0, 1, 0); local dist = (startPos - targetPos).Magnitude; local heightDiff = targetPos.Y - startPos.Y; local horizontalDist = (Vector3.new(targetPos.X, 0, targetPos.Z) - Vector3.new(startPos.X, 0, startPos.Z)).Magnitude; local combo = {}
    ...
    local dummyHistory = { {pos = startPos}, {pos = targetPos} }
    return dummyHistory, table.concat(combo, " -> ")
end
function RouteGenerator:CompressHistory(history: table): string
    local compressed = {}; local currAction = history[1]; local count = 0
    for i=1, #history do
        if history[i] == currAction then
            count += 1
        else
            table.insert(compressed, currAction .. "(" .. count .. ")"); currAction = history[i]; count = 1
        end
    end
...
function PointManager:Initialize(workspaceRoot)
    print("WHOODMMYLEGSS"); MapAnalyzer:InitMap(workspaceRoot)
end
...
function RouteUI:UpdateMarkers()
    ...
    for i, data in PointManager.Points do
        ...
        if i == #PointManager.Points then
            local stroke = Instance.new("UIStroke", bg); stroke.Color = THEME.AccentColor; stroke.Thickness = 2; local label = Instance.new("TextLabel", bb); label.Text = "point " .. i; label.Position = UDim2.new(0, 0, 0, -20); label.Size = UDim2.new(1, 0, 0, 20); label.BackgroundTransparency = 1; label.TextColor3 = Color3.new(1,1,1); label.Font = Enum.Font.Code; label.TextSize = 10
        end
        local num = Instance.new("TextLabel", bg); num.Text = tostring(i); num.Size = UDim2.new(1,0,1,0); num.BackgroundTransparency = 1; num.TextColor3 = Color3.new(1,1,1); num.Font = Enum.Font.Code; num.TextSize = 12
    end
end
										...
										if targetPart:IsA("BasePart") or targetPart:IsA("MeshPart") then
											...
											if animData and animData:IsA("ModuleScript") and targetPart.Transparency ~= 1 then
												...
												if delays then
													...
													for _, child in pairs(animData:GetChildren()) do
				...
				if gloveName and gloveName ~= "None" and gloveSkin and GloveModels:FindFirstChild(gloveName) then
					...
					local function applyGloveAnim(glovePart)
						if gloveAnimData and gloveAnimData:IsA("ModuleScript") then
							...
							if delays then
								local frames, maxFrame = {}, 0
								for _, child in pairs(gloveAnimData:GetChildren()) do
									local frameNum = tonumber(child.Name)
									if frameNum then
										local frameTex = nil
										if child:IsA("StringValue") and child.Value ~= "" and child.Value ~= "rbxassetid://0" then frameTex = child.Value
...
luaTab = UI_Library:AddTab("LUA"); playersTab = UI_Library:AddTab("Players")
	...
	function startDemoRecording()
		...
		print("Started recording to " .. currentDemoFolder); demoStatusMode = "recording"
		if demoStatusLabel and demoStatusLabel.main then
			demoStatusLabel.main.Text = "Status: Recording...."
		end
	end
	function stopDemoRecording()
		if not isRecordingDemos then return end
		isRecordingDemos = false; demoStatusMode = "saving"
		for i, frameData in val_494 do
			...
			if demoStatusLabel and demoStatusLabel.main then
				demoStatusLabel.main.Text = "Status: Creating " .. tostring(i) .. ".tick"
			end
		end
		...
		local demoVer = workspace:FindFirstChild("version") and workspace.version.Value or ""; writefile(currentDemoFolder .. "/ver.tick", tostring(demoVer)); val_494 = {}; demoStatusMode = "idle"
		if demoStatusLabel and demoStatusLabel.main then
	...
	demoSecRecorder:AddButton({
		...
		callback = function()
			...
			if demoStatusLabel and demoStatusLabel.main and demoStatusMode == "idle" then
				demoLinkCopiedUntil = os.clock() + 2; demoStatusLabel.main.Text = "Status: Link copied!"; demoStatusLabel.main.TextColor3 = Color3.fromRGB(120, 200, 255)
			end
		end
	})
	...
	task.spawn(function()
		...
		while true do
			if isRecordingDemos then
				pcall(addTick); nextTick = nextTick + DEMO_TICK_INTERVAL; local now = os.clock()
...
function addTick()
		...
		for var_155, var_218 in val_491:GetPlayers() do
			...
			xpcall(function()
				...
				warn("[Demo Recorder] Error recording player " .. tostring(var_218.Name) .. ": " .. tostring(err2))
			end)
		end 
...
UI_Library.Init = function(self)
				...
				for _, opt in sec.options or {} do
					...
					if opt.type == "toggle" then
						...
						if associatedBind then
							...
							bindBtn.MouseButton1Click:Connect(function()
								listening = not listening; bindBtn.Text = listening and "..." or shortenBindName(associatedBind.key or "none")
							end)
							...
							game:GetService("UserInputService").InputBegan:Connect(function(input, processed)
								if listening and not processed then
									...
									if kName then
										associatedBind.key = kName; bindBtn.Text = kName; listening = false
									end
								end
							end)
					...
					elseif opt.type == "keybind" or opt.type == "bind" then
						...
						if isChained then
							...
							bindBtn.MouseButton1Click:Connect(function()
								listening = not listening; bindBtn.Text = listening and "..." or shortenBindName(opt.key or opt.state or "none")
							end)
							game:GetService("UserInputService").InputBegan:Connect(function(input, processed)
								if listening and not processed then
									local kName = input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode.Name or (input.UserInputType.Name:find("MouseButton") and "M" .. input.UserInputType.Name:sub(-1) or nil)
									if kName then
										opt.key = kName; opt.state = kName; bindBtn.Text = kName; listening = false; library_flags[opt.flag] = kName
									end
								end
							end)
							local runConnection = nil
	...
	game:GetService("RunService").RenderStepped:Connect(function()
		if self.open then
			game:GetService("UserInputService").MouseIconEnabled = false; game:GetService("UserInputService").MouseBehavior = Enum.MouseBehavior.Default; local mPos = game:GetService("UserInputService"):GetMouseLocation(); local p = Vector2.new(mPos.X, mPos.Y); env._cursor_outer.Position = p; env._cursor_inner.Position = p; env._cursor_outer.Visible = true; env._cursor_inner.Visible = true
		end
	end)
	game:GetService("UserInputService").InputBegan:Connect(function(input, processed)
...
UI_Library:Init(); corehusband = Instance.new("ScreenGui", game:service("CoreGui")); logholder = game:GetObjects("rbxassetid://6502006065")[1]; logholder.log.main.BackgroundColor3 = Color3.fromRGB(25, 25, 25); logholder.log.main.text.TextColor3 = Color3.new(0.8, 0.8, 0.8); logholder.Parent = corehusband; logcount = 0; logDebounce = false 
...
for var_200, var_245 in val_711:GetPlayers() do
	UI_Library:AddConnection(var_245.CharacterAdded, function(var_43)
		...
		if library_flags["Backtrack"] and var_245 ~= val_733 then
			createPlr(var_245)
		end 
...
UI_Library:AddConnection(val_711.PlayerAdded, function(var_57)
	UI_Library:AddConnection(var_57.CharacterAdded, function(var_264)
		wait(1); val_823 = true 
		if library_flags["Backtrack"] and var_57 ~= val_733 then
			createPlr(var_57)
		end 
...
local function updateEdgeBox(pos, active, color)
		if not edgeBox then
			edgeBox = Instance.new("Part"); edgeBox.Name = "EB_Visual"; edgeBox.Shape = Enum.PartType.Block
			edgeBox.CastShadow = false
			edgeBox.CanCollide = false; edgeBox.Anchored = true; edgeBox.Material = Enum.Material.SmoothPlastic; edgeBox.Size = Vector3.new(1.3, 1.3, 1.3); edgeBox.Parent = workspace:FindFirstChild("Ray_Ignore") or workspace; local outline = Instance.new("SelectionBox"); outline.Name = "EB_Outline"
			outline.Adornee = edgeBox
			outline.LineThickness = 0.05; outline.Color3 = Color3.new(0, 0, 0); outline.Parent = edgeBox
		end
		edgeBox.Position = pos; edgeBox.Color = Color3.new(0, 0, 0); edgeBox.Transparency = 1; local isViz = library_flags["Eb_Visualize"] ~= false
	...
	pcall(function()
		...
		if ebMode == "mimic" and ebEnabled and (autoStatus or env.ebHolding) then
			if env.mimicHum ~= hum then
				...
				env.mimicStateConn = hum.StateChanged:Connect(function(old, new)
					...
					if env.mimicWasFalling and new == Enum.HumanoidStateType.Landed and not edgebugDebounce then
						env.mimicWasFalling = false; edgebugDebounce = true; env.hookJP = 0; local currentHrp = val_733.Character and val_733.Character:FindFirstChild("HumanoidRootPart"); local currentHum = val_733.Character and val_733.Character:FindFirstChild("Humanoid")
						...
						spawn(function()
							...
							local vel = currentHrp.AssemblyLinearVelocity; currentHrp.AssemblyLinearVelocity = Vector3.new(vel.X * 1.8, -7, vel.Z * 1.8); local vel2 = currentHrp.AssemblyLinearVelocity
							for i = 1, 4 do
								wait()
								if not currentHrp then return end
							...
							wait()
							if not currentHrp then return end
							currentHrp.AssemblyLinearVelocity = currentHrp.AssemblyLinearVelocity * Vector3.new(1.8, 1, 1.8); env.hookJP = nil
							delay(0.3, function()
			...
			if env.mimicStateConn and ebMode ~= "mimic" then
				env.mimicStateConn:Disconnect(); env.mimicStateConn = nil; env.mimicHum = nil
			end
		end
...
(underRay and forwardRay and (underRay.Position.Y - forwardRay.Position.Y) > 1.0)
		...
		if isGrounded then
			env.jbWasGrounded = true; env.hasBoosted = nil; env.jbAirFrames = 0
		else
			env.jbAirFrames = (env.jbAirFrames or 0) + 1
		end
		if library_flags["Jumpbug"] and env.jbBindHeld then
			if not isGrounded and env.jbWasGrounded and not env.hasBoosted
				and (env.jbAirFrames or 0) >= 3 and velocity.Y > 0 and velocity.Y < 15
				and (tick() - (env.jbTriggerTime or 0)) > 0.3 then
				local boostVal = library_flags["jbheight"] or 3.5; local targetY = boostVal * 5.5; local newY = math.min(velocity.Y + targetY, targetY + 2); hrp.AssemblyLinearVelocity = Vector3.new(velocity.X, newY, velocity.Z); env.hasBoosted = true; env.jbWasGrounded = false; env.jbTriggerTime = tick(); env.lastJumpbugTime = tick()
			end
		end
	end)
			...
			if val_566 and teamCheck(val_711[var_101], val_772) and val_711[var_101].Character and val_711[var_101].Character:FindFirstChild("Humanoid") and val_711[var_101].Character:FindFirstChild("HumanoidRootPart") and val_711[var_101].Character:FindFirstChild("Head") or not val_566 and var_223.object then
				...
				if var_223.flags then
					if library_flags["Flags ESP"] and val_567 then
						...
						if _on("Bomb") and ((workspace:FindFirstChild("Status") and workspace.Status:FindFirstChild("HasBomb") and workspace.Status.HasBomb.Value == val_567.Name) or (ch and ch:FindFirstChild("BackC4") and ch.BackC4.Transparency < 1)) then
							fl[#fl+1] = "Bomb"
						end
						if ch then
	...
	pcall(function()
		local enabled = library_flags["Like Gecko"] or library_flags["Wallclimb"] or env.wcBindHeld
		if enabled then
			local hrp = val_733.Character and val_733.Character:FindFirstChild("HumanoidRootPart"); local hum = val_733.Character and val_733.Character:FindFirstChild("Humanoid")
			if hrp and hum then
				local isClimbingLadder = false; local vf = val_733.Character:FindFirstChildWhichIsA("VectorForce", true)
				if vf and vf.Enabled then isClimbingLadder = true end
				if not isClimbingLadder then
					local now = tick()
					if not env.lastWallclimbTime then env.lastWallclimbTime = 0 end
					if (now - env.lastWallclimbTime) >= 0.25 then
						local rayParams = RaycastParams.new()
						rayParams.FilterDescendantsInstances = {val_733.Character, workspace:FindFirstChild("Ray_Ignore")}
						rayParams.FilterType = Enum.RaycastFilterType.Exclude; local center = hrp.Position; local lv = Vector3.new(hrp.CFrame.LookVector.X, 0, hrp.CFrame.LookVector.Z).Unit; local rv = Vector3.new(hrp.CFrame.RightVector.X, 0, hrp.CFrame.RightVector.Z).Unit; local baseDirs = { lv, -lv, rv, -rv }; local dirs = {}
						for _, bDir in baseDirs do
							table.insert(dirs, bDir); table.insert(dirs, (CFrame.new(Vector3.zero, bDir) * CFrame.Angles(0, math.rad(20), 0)).LookVector); table.insert(dirs, (CFrame.new(Vector3.zero, bDir) * CFrame.Angles(0, math.rad(-20), 0)).LookVector)
						end
						local hitFound = false
						for _, dir in dirs do
							local pos1 = center + Vector3.new(0, 0.5, 0); local pos2 = center - Vector3.new(0, 0.5, 0); local hit1 = workspace:Raycast(pos1, dir * 4.0, rayParams); local hit2 = workspace:Raycast(pos2, dir * 4.0, rayParams)
							if hit1 or hit2 then
								hitFound = true; break
							end
						end
						if hitFound then
							env.wallclimbing = true; local currentVel = hrp.AssemblyLinearVelocity; hrp.AssemblyLinearVelocity = Vector3.new(currentVel.X, 35, currentVel.Z)
							pcall(function()
	...
	if library_flags["Bunny Hop"] and val_872.alive and not val_858 then
	...
	if val_671:IsMouseButtonPressed(0) and env.silentAimEnabled then
		...
		-- We no longer integrate backtrack into silent aim's target selection here.
		-- They operate completely independently now.
...
;(function()
FiremanConfig = { boostForce = 15, upwardBoost = 3, cooldown = 0.2 }
...
game:GetService("RunService").Heartbeat:Connect(function(dt)
    ...
    if not env.fmActiveHold then
        canBoost = false; env.fmTriggered = false; wasOnLadder = false; return
    end
    local plr = game.Players.LocalPlayer; local char = plr.Character
    ...
    if isOnLadder and library_flags["Fireman"] then
        ...
        if dir.Magnitude > 0.1 then
            dir = dir.Unit; local horizontalVel = Vector3.new(vel.X, 0, vel.Z); local horizontalSpeed = horizontalVel.Magnitude; local boostedHorizontal = dir * math.max(horizontalSpeed, 16); local verticalBoost = vel.Y * 2.5; hrp.AssemblyLinearVelocity = Vector3.new(boostedHorizontal.X, verticalBoost, boostedHorizontal.Z)
        end
        env.fmTriggered = false; canBoost = true
    elseif wasOnLadder and not isOnLadder and canBoost and library_flags["Fireman"] then
        if (tick() - lastCooldown) > FiremanConfig.cooldown then
            lastCooldown = tick()
            canBoost = false; local dir = hum.MoveDirection
            if dir.Magnitude < 0.1 then
                dir = hrp.CFrame.LookVector * Vector3.new(1,0,1)
            end
            if dir.Magnitude > 0 then dir = dir.Unit end
            local speed = (vel * Vector3.new(1, 0, 1)).Magnitude; local boostVel = dir * (speed + FiremanConfig.boostForce); local verticalBoost = vel.Y < 0 and FiremanConfig.upwardBoost or 15; hrp.AssemblyLinearVelocity = Vector3.new(boostVel.X, verticalBoost, boostVel.Z); env.fmTriggered = true
            task.delay(0.5, function()
                env.fmTriggered = false; canBoost = false
            end)
        end
    else
...
task.spawn(function()
    local cachedObjects = {}; local lastCacheTime = 0; local CACHE_INTERVAL = 10
    while task.wait(1) do
        ...
        if not val_414 then continue end
        local playerName = val_414.Name; local displayName = val_414.DisplayName; local now = tick()
        if now - lastCacheTime > CACHE_INTERVAL then
            ...
            pcall(function()
                ...
                local pg = val_414:FindFirstChild("PlayerGui")
                if pg then
...
if game.PlaceId == 101013872711019 then
	...
	game:GetService("RunService").Stepped:Connect(function()
		for L_477_forvar0, L_478_forvar1 in workspace.demoModels:GetChildren() do
